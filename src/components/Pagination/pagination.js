...
const Pagination = ({ limit, pages, currentPage, setCurrentPage }) => {
  ...
  return (
    <div className="pagination m-2 mx-auto flex justify-center">
      <button
        onClick={
          currentPage !== 1
            ? () => {
          ...
          if (index <= 2 || index >= pages.length - 2 || (index >= currentPage - 2 && index <= currentPage + 2)) {
            return (
              <button
                ...
                style={{
                  padding: "2px 10px",
                  background: page === currentPage ? "black" : "#F5F5F5",
                  outline: "none",
                  border: "none",
                  borderRadius: "4px",
                  border: page === currentPage ? "1px solid black" : "1px solid #EEEEEE",
      ...
      <button
        ...
        className={`mt-3`}
      >
        <IoIosArrowForward className="mb-1 ml-3" />
      </button>
    </div>
  );
};
